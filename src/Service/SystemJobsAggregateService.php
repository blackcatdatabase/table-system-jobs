 ...
 */
final class SystemJobsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private SystemJobRepository $systemJobRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'system_jobs' => $this->systemJobRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\SystemJobs\ModuleException("Record {$idStr} not found for locking.");
            }
